...
\subsection{DeepFool} % WHAT you chose, WHY, EXPECTATIONS for runtime/performance tradeoffs, how to generate testing pairs for ML alg, what knowledge represenations/why, why CNN over transformer, training/eval time, knowledge representation and expected accuracy
The DeepFool algorithm is used to misclassify an image by computing the minimal perturbations required in the most efficient manner. The authors Moosavi-Dezfooli et al. [2] performed theirt testing alongside the FGSM method in order to compare results and found that although both could misclassify the original image, FGSM required more perturbations and was thus more noticeable than DeepFool.  
